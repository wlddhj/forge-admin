...
- `type=3` 条件审批 — 单独使用时行为近似 type=1,**conditionList 不生效**
- `type=4` 条件分支 — 排他网关,按 `conditionList` 路由到不同 childNode

**关键澄清**:条件路由的正确做法是 **type=4 + 多个 type=3 子节点**(每个子节点代表一个"条件+审批人"组合)。设计器中"条件分支"对应 type=4,可加多个子节点。

典型组合: **触发器节点写变量 → 条件分支读变量路由**。完整细节见 [02.condition-node-guide.md](02.condition-node-guide.md)。
...
**完整开发指南**: [03.trigger-bean-guide.md](03.trigger-bean-guide.md)
...
详见 [03.trigger-bean-guide.md §8](03.trigger-bean-guide.md#8-返回值约定与异常处理)。
...
> ⚠️ **当前状态**: 此模块是 **配置展示入口**,FlowLong 引擎当前不会读取 `wf_process_listener` 表。实际运行中的监听器由项目内的 `@Component` Bean(`taskListener` / `instanceListener` 等)提供。**在此页面增删改数据不会影响流程执行行为**。

> 💡 **注意区分**:本节说的"监听器"是**流程级监听器**(执行于 `start` / `end` / `create` 等事件),与 §7 的**触发器任务**(独立流程节点,type=7)是不同概念。如需配置触发器(写流程变量、动态 Bean 调用等),用 §7;此处是历史占位功能。
...
A: **当前没有**。此模块是配置展示入口,运行时由 `@Component` Bean 接管。详见第 11 节。

### Q9: 拖出 type=3 条件审批节点,配置的 conditionList 不生效?

A: 这是**正常行为**。type=3 单独使用时,`TaskServiceImpl` 的处理逻辑与 type=1 approval **完全相同**(直接生成 task,不读 conditionList)。要实现条件路由,正确做法是拖出 **type=4 条件分支 + 多个 type=3 子节点**,每个子节点的 conditionList 才会被引擎读取。详见 [02.condition-node-guide.md §2.2](02.condition-node-guide.md#22-作为独立节点--条件不生效)。
